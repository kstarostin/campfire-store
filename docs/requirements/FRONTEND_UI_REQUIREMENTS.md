...
| Icons        | **lucide-react**                                | Same icon set as `docs/mockups/homepage.html`                         |
...
## 3. Design reference (`homepage.html`)

Open `docs/mockups/homepage.html` in a browser alongside this doc. Replicate:
...
Load fonts via Google Fonts (same URL as `docs/mockups/homepage.html`) or self-host.
...
| Favicon         | `favicon.ico`             | Site favicon — reference in `docs/mockups/homepage.html` as `<link rel="icon" href="/favicon.ico" />` |
...
Match `docs/mockups/homepage.html` footer grid. Background: `var(--color-footer-bg)` (`#f0f0f0`).
...
├── docs/
│   └── mockups/
│       └── homepage.html               # Vite entry (not the design mockup)
├── public/
│   └── img/                 # logo assets
├── src/
...
- [ ] Design tokens match §4 and visual check against `docs/mockups/homepage.html`
